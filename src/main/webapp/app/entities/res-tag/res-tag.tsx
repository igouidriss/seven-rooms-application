...
import { getEntities } from './res-tag.reducer';
...
export const ResTag = () => {
  ...
  const resTagList = useAppSelector(state => state.resTag.entities);
  const loading = useAppSelector(state => state.resTag.loading);
  const totalItems = useAppSelector(state => state.resTag.totalItems);
  ...
  return (
    <div>
      <h2 id="res-tag-heading" data-cy="ResTagHeading">
        <Translate contentKey="sevenRoomsToHubApplicationApp.resTag.home.title">Res Tags</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="sevenRoomsToHubApplicationApp.resTag.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/res-tag/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="sevenRoomsToHubApplicationApp.resTag.home.createLabel">Create new Res Tag</Translate>
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        {resTagList && resTagList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="sevenRoomsToHubApplicationApp.resTag.id">ID</Translate>{' '}
            ...
            <tbody>
              {resTagList.map((resTag, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/res-tag/${resTag.id}`} color="link" size="sm">
                      {resTag.id}
                    </Button>
                  </td>
                  <td>{resTag.tag}</td>
                  ...
                  <td>
                    {resTag.techCreatedDate ? <TextFormat type="date" value={resTag.techCreatedDate} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    {resTag.techUpdatedDate ? <TextFormat type="date" value={resTag.techUpdatedDate} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>{resTag.techMapping}</td>
                  <td>{resTag.techComment}</td>
                  <td>{resTag.reservation ? <Link to={`/reservation/${resTag.reservation.id}`}>{resTag.reservation.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/res-tag/${resTag.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        tag={Link}
                        to={`/res-tag/${resTag.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/res-tag/${resTag.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="sevenRoomsToHubApplicationApp.resTag.home.notFound">No Res Tags found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={resTagList && resTagList.length > 0 ? '' : 'd-none'}>
