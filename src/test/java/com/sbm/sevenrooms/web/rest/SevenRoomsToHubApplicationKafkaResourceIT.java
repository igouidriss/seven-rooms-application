...
import com.sbm.sevenrooms.config.EmbeddedKafka;
...
class SevenRoomsToHubApplicationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/seven-rooms-to-hub-application-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/seven-rooms-to-hub-application-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/seven-rooms-to-hub-application-kafka/unregister"));
                return;
            }
        }
